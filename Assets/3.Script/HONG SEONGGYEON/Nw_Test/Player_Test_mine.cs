...
enum PlayerType_mine
{
    SPEED,
    SCALE
};

public class Player_Test_mine : NetworkBehaviour
{
    [SerializeField]
    ...
    private PlayerType_mine playerType;
    Animator player_Ani;
    ...
    {if (!isLocalPlayer) return;
        ...
        if (choosePlayer == 0) playerType = PlayerType_mine.SPEED;
        else playerType = PlayerType_mine.SCALE;
    ...
    public void UseSkill()
    {
        ...
        if (Input.GetKeyDown(KeyCode.E) && Time.time >= nextSkillTime)  // 스킬 쿨타임 체크
        {
            switch (playerType)
            {
                case PlayerType_mine.SPEED:
                    StartCoroutine(SpeedUP_co());
                    break;

                case PlayerType_mine.SCALE:
